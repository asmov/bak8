...
impl SourceTraitBackupPath {
    ...
    pub fn read_fs_version(&self) -> BackupResult<semver::Version> {
        ...
        fs::read_to_string(self.as_path())
            .map_err(|e| BackupError::file_io(e, self.as_path(), "Failed to read file system version file"))
            .map(|version_str| semver::Version::parse(&version_str))?
            .map_err(|e| BackupError::file_io(e, self.as_path(), "Failed to parse file system version file"))
    }
    ...
    pub fn setup(&self, config: &BackupConfig) -> BackupResult<()> {
        ...
        match self {
            Self::StorageDir(path) => {
                if !path.exists() {
                    fs::create_dir_all(path)
                        .map_err(|e| BackupError::file_io(e, path, "Failed to create backup storage directory"))?;
                    cross::PLATFORM.fs()
                        .own_capable(&path, admin_uaid, &backup_users_gaid_capable, BACKUP_STORAGE_DIR_PERMISSIONS)
                        .map_err(|e| BackupError::file_io(e, &path, "Failed to set permissions on backup storage directory"))?;
                }

                let backup_storage_dir = path.canonicalize()
                    .map_err(|e| BackupError::file_io(e, path, "Backup storage directory is not accessible"))?;
                ...
                for subdir in backup_storage_subdirs(&backup_storage_dir) {
                    if !subdir.exists() {
                        fs::create_dir(&subdir)
                            .map_err(|e| BackupError::file_io(e, &subdir, "Failed to create backup storage subdirectory"))?;
                    }
                    
                    cross::PLATFORM.fs()
                        .own_capable(&subdir, admin_uaid, &backup_users_gaid_capable, BACKUP_STORAGE_DIR_PERMISSIONS)
                        .map_err(|e| BackupError::file_io(e, &subdir, "Failed to set permissions on backup storage subdirectory"))?;
                    ...
                    for backup_user in &all_backup_users {
                        ...
                        if !user_subdir.exists() {
                            fs::create_dir(&user_subdir)
                                .map_err(|e| BackupError::file_io(e, &user_subdir, "Failed to create backup storage subdirectory"))?;
                        }
                        ...
                        cross::PLATFORM.fs()
                            ...
                            .map_err(|e| BackupError::file_io(e, &user_subdir, "Failed to set permissions on user backup storage subdirectory"))?;
                    }
                }
            ...
            Self::UserConfig { path, .. } => {
                if !path.exists() {
                    let dir = path.parent()
                        .ok_or_else(|| BackupError::file_io_err(path, "Unable to determine user config directory"))?;
                    fs::create_dir_all(dir)
                        .map_err(|e| BackupError::file_io(e, path, "Failed to create user config directory"))?;
                    fs::write(path, config::CONFIG_DEFAULTS)
                        .map_err(|e| BackupError::file_io(e, path, "Failed to create default user config file"))?;
                }
            },
            Self::FileSytemVersion { path, .. } => {
                if !path.exists() {
                    fs::write(path, crate::consts::SOURCETRAIT_BACKUP_FS_VERSION.to_string())
                        .map_err(|e| BackupError::file_io(e, path, "Failed to create file system version file"))?;
                    cross::PLATFORM.fs()
                        .own_capable(&path, admin_uaid, &backup_users_gaid_capable, BACKUP_STORAGE_FILE_PERMISSIONS)
                        .map_err(|e| BackupError::file_io(e, &path, "Failed to set permissions on file system version file"))?;
                }
            },
            Self::RemoteStorageDir { remote, storage_dir } => {
                ...
                if !crate::cmd::ssh_run::ssh_dirs_exist(remote, &backup_storage_check_paths(&storage_dir))? {
                    ...
                    crate::cmd::ssh_run::ssh_make_dirs(remote, &dirs)
                        .map_err(|e| BackupError::remote_cmd(e, remote, &format!(
                    ...
                    let remote_fs_version = crate::cmd::ssh_run::ssh_file_contents(remote, &fs_version_file)?
                        .ok_or_else(|| BackupError::remote_cmd_err(remote, &format!(
                    ...
                    if !crate::consts::SOURCETRAIT_BACKUP_FS_VERSION_REQ.matches(&remote_fs_version) {
                        return Err(BackupError::remote_cmd_err(remote, &format!(
                        ...
                        if !crate::cmd::ssh_run::ssh_dirs_exist(remote, &dirs)? {
                            //todo: warn
                            crate::cmd::ssh_run::ssh_make_dirs(remote, &dirs)
                                .map_err(|e| BackupError::remote_cmd(e, remote,
    ...
    fn verify_storage_dir(storage_dir: &Path) -> BackupResult<()> {
        match storage_dir.exists() {
            true => Ok(()),
            false => Err(BackupError::file_io_err(storage_dir, "Backup storage directory does not exist"))
        }
    }
    ...
    pub fn prepare(&self, _config: &BackupConfig) -> BackupResult<()> {
        ...
        match self {
                ...
                | Self::Archive { storage_dir, path, .. } =>
            {
                ...
                if !path.exists() {
                    ...
                    if !user_subdir.exists() {
                        fs::create_dir(&user_subdir)
                            .map_err(|e| BackupError::file_io(e, &user_subdir, "Failed to create context subdirectory for user"))?;
                        ...
                        cross::PLATFORM.fs()
                            ...
                            .map_err(|e| BackupError::file_io(e, &user_subdir, "Failed to set ownership for user context subdirectory"))?;
                    ...
                    if !host_subdir.exists() {
                        fs::create_dir(&host_subdir)
                            .map_err(|e| BackupError::file_io(e, &host_subdir, "Failed to create context subdirectory for host"))?;
                        cross::PLATFORM.fs()
                            .own_capable(&host_subdir, &uaid, &gaid_capable, BACKUP_STORAGE_DIR_PERMISSIONS)
                            .map_err(|e| BackupError::file_io(e, &host_subdir, "Failed to set ownership for host context subdirectory"))?;
                    }
                    ...
                    if !year_subdir.exists() {
                        fs::create_dir(&year_subdir)
                            .map_err(|e| BackupError::file_io(e, &year_subdir, "Failed to create context subdirectory for year"))?;
                        ...
                        cross::PLATFORM.fs()
                            ...
                            .map_err(|e| BackupError::file_io(e, &year_subdir, "Failed to set ownership for year context subdirectory"))?;
                    ...
                    if !month_subdir.exists() {
                        fs::create_dir(&month_subdir)
                            .map_err(|e| BackupError::file_io(e, &month_subdir, "Failed to create context subdirectory for month"))?;
                        cross::PLATFORM.fs()
                            .own_capable(&year_subdir, &uaid, &gaid_capable, BACKUP_RUN_DIR_PERMISSIONS)
                            .map_err(|e| BackupError::file_io(e, &month_subdir, "Failed to set ownership for month context subdirectory"))?;
                    ...
                    if !day_subdir.exists() {
                        fs::create_dir(&day_subdir)
                            .map_err(|e| BackupError::file_io(e, &day_subdir, "Failed to create context subdirectory for day"))?;
                        cross::PLATFORM.fs()
                            .own_capable(&day_subdir, uaid, &gaid_capable, BACKUP_RUN_DIR_PERMISSIONS)
                            .map_err(|e| BackupError::file_io(e, &day_subdir, "Failed to set ownership for day context subdirectory"))?;
...
pub fn expand_path(path_str: &str) -> BackupResult<PathBuf> {
    Ok(crate::sys::expand_env(path_str)?.to_string().into())
}

pub fn home_dir() -> BackupResult<PathBuf> {
...
#[allow(dead_code)]
pub fn setup_home_config(force: bool) -> BackupResult<()> {
    ...
    if !home_config_dir.exists() {
        fs::create_dir_all(&home_config_dir)
            .map_err(|e| BackupError::file_io(e, &home_config_dir, "Unable to create user config directory"))?;
    }

    if !home_config_file.exists() || force {
        fs::write(&home_config_file, config::CONFIG_DEFAULTS)
            .map_err(|e| BackupError::file_io(e, &home_config_file, "Unable to create default user config file"))?;
    }
...
pub fn verify_backup_dirs(config: &config::BackupConfig) -> BackupResult<()> {
    let backup_storage_dir = &config.backup_storage_dir_path();
    let backup_storage_dir = backup_storage_dir.canonicalize()
        .map_err(|e| BackupError::configured_dir(&backup_storage_dir, CFG_BACKUP_STORAGE_DIR, e))?;

    for subdir in backup_storage_subdirs(&backup_storage_dir) {
        subdir.canonicalize()
            .map_err(|e| BackupError::configured_subdir(&subdir, CFG_BACKUP_STORAGE_DIR, e))?;
    }
