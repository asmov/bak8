...
pub fn cmd_rsync_full(source_dir: &Path, dest_dir: &Path) -> std::process::Command {
    ...
    cmd.args(&[
        RSYNC_FLAG_ARCHIVE,
        &format!("{}/", source_dir.to_str().unwrap()),
...
pub fn cmd_rsync_full_ssh(source_dir: &Path, remote: &Remote, remote_dest_dir: &Path) -> std::process::Command {
    let mut cmd = std::process::Command::new(RSYNC_CMD);
    let source_dir = format!("{}/", source_dir.to_str().unwrap());
    let remote_dest_dir = format!("{}:{}", remote.addr(), remote_dest_dir.to_str().unwrap());
    cmd.args(&[
        RSYNC_FLAG_ARCHIVE,
        &source_dir,
        &remote_dest_dir,
...
pub fn cmd_rsync_incremental(source_dir: &Path, incremental_source_dir: &Path, dest_dir: &Path) -> std::process::Command {
    ...
    cmd.args(&[
        ...
        &format!("{}/", incremental_source_dir.to_str().unwrap()),
        &format!("{}/", source_dir.to_str().unwrap()),
        dest_dir.to_str().unwrap(),
    ]);
