...
impl CrossPlatform for LinuxCrossPlatform {
    fn in_terminal(&self) -> CrossResult<bool> {
        match env::var(ENV_XDG_SESSION_TYPE) {
            Ok(_) => Ok(true),
            Err(env::VarError::NotPresent) => Ok(false),
            Err(source) => CrossError::err_env_var(ENV_VAR_XDG_SESSION_TYPE, source),
        }
    }
        ...
        P: AsRef<Path> + Into<PathBuf>,
    {
        let sys_open_cmd = match self.in_terminal()? {
            true => None,
            false => Some(cmd_xdg_open(file.as_ref()))
        };
...
fn cmd_xdg_open<P>(file: P) -> process::Command
where
    P: AsRef<Path> + Into<PathBuf>,
{
...
where
    ...
    P2: AsRef<Path> + Into<PathBuf>,
{
    const CP_ARG_PRESERVE: &'static str = "--preserve";
